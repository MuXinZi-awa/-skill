...
**修复**
- `heihe_post.py` 发帖成功后**自动记录 link_id**：此前 `log_action("发帖", "", ...)` 把 link 记成空串，导致水贴记录里 link 栏一直是空的、发完帖找不回链接（2026-08-28 密教三帖因此 link 缺失）。现在从发帖响应提取 `result.linkid` 写入日志，发帖即输出 `[发帖] link-id: xxx`（实测 link 189360525）

**其他**
- 帖子库新增第 5 篇：《30 块的晚饭，和那个自洽的人》（杂谈区，已发布，link 189360525）
